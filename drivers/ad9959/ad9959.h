/*
 * Simple (and imcomplete) driver for Analog Devices AD9959 DDS chip
 *
 * This library assumes that you are pulling down the power down pin externally. If you are not
 * doing so, nothing will work because the chip won't be turned out.
 *
...
public:
  ...
  void set_freq_linear_sweep_params(Channel ch,
...
private:
  ...
  // the datasheet to run properly on a little endian system. There are no checks
  // nor compatible implementation for big endian systems at tihs time.
  // TODO(bolamalek): add edianess check and big endian support.
  typedef union {
    struct {
  ...
  typedef union {
    struct {
      ...
      uint8_t open_sync :2; // See Synchronizing Multiple AD9959 Decives section for details.
      ...
      uint8_t pll_divider_ratio :5; // value between 4 and 20 to enable PLL. Otherwise, disable PLL
  ...
  typedef union {
    struct {
      ...
      uint8_t multipledevice_sync_master :1; // See Synchronizing Multiple AD9959 Devices
      uint8_t multidevice_sync_status :1; // See Synchronizing Multiple AD9959 Devices
      uint8_t multidevice_sync_master_enable :1; // See Synchronizing Multiple AD9959 Devices
  ...
  typedef union {
    struct {
      ...
      uint8_t clear_phase_accumulator :1; // 0: normal operation (default), 1: asynchronous clear
      uint8_t autoclear_phase_accumulator :1; // 0: no change on io_update (default), 1:cleaned and synced on io_update
      uint8_t clear_sweep_accumulator :1; // 0: normal operation (default), 1: asynchronous clear
      uint8_t autoclear_sweep_accumulator :1; // 0: no change on io_update (default), 1:cleaned and synced on io_update
      ...
      uint8_t dac_full_scale_current_control :2; // 0b11: the DAC is at the largest LSB value (default)
                             // See Table 5 for other settings.
      uint8_t must_be_zero :1; // Keep at at 0
      uint8_t :2; // Open, do not use.
      uint8_t load_srr_at_io_update :1; // 0: loaded only on timeout(default), 1: loaded on timeout and ioupdate.
      uint8_t linear_sweep_enable :1; // 0: disable(defualt), 1: enaled
      uint8_t linear_sweep_no_dwell :1; // 0 no-dwell inactive (default), 1: no-dwell active.
  ...
  typedef union {
    struct {
      ...
      uint16_t load_arr_at_io_update :1; // 0: loaded only on timeout(default), 1: loaded on timeout and ioupdate.
      uint16_t ramp_up_down_enable :1; // only used when amplitude_multiplier_enable is 1
                       // 0: enabled manuel ru_rd operation (default), 1:auto ru_rd operation
      uint16_t amplitude_multiplier_enable :1; // 0: disabled(default), 1: enabled
